...
class AlbertEmbeddings(BertEmbeddings):
    ...
    def __init__(self, config):
        ...
        self.utterance_type_embeddings = nn.Embedding(4, config.embedding_size)
...
class LabelSmoothingLoss(nn.Module):
    ...
    def __init__(self, label_smoothing, tgt_vocab_size, ignore_index=None):
        ...
        # print('ignored index is in the last spot self.unk')
        self.ignore_index = ignore_index
        self.ignore_index_idx = tgt_vocab_size - 1
        ...
        if ignore_index is not None:
            one_hot[self.ignore_index_idx] = 0
    ...
    def forward(self, output, target):
        ...
        # print('@@@', output.shape, model_prob.shape, target.max(), '###')
        # if int(target.min()) == -100:
        #     print('WHAT')
        if self.ignore_index is not None:
            target[target == self.ignore_index] = self.ignore_index_idx # for making -100 go to last index
            model_prob.scatter_(1, target.unsqueeze(1), self.confidence)
            model_prob.masked_fill_((target == self.ignore_index_idx).unsqueeze(1), 0)
        else:
            model_prob.scatter_(1, target.unsqueeze(1), self.confidence)

        result = torch.mean(torch.sum(-model_prob * output, dim=-1))

        return result

class LabelSmoothingLoss(nn.Module):

    def __init__(self, eps , class_size, ignore_index=-100, reduction='mean'):
        super().__init__()
...
# class LabelSmoothingLoss(nn.Module):
#     def __init__(self, tgt_vocab_size, smoothing=0.0, dim=-1):
#         super(LabelSmoothingLoss, self).__init__()
#         self.ignore_index = -100
#         self.ignore_index = tgt_vocab_size - 2
    ...
    def forward(
        ...
        **kwargs,
        ...
        if gate_labels is not None:
            ...
            gate_loss = loss_fct(domain_gate_prediction_scores.reshape(-1, self.num_gate_label), gate_labels.view(-1, self.num_gate_label).to(domain_gate_prediction_scores.dtype))
