...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        if (
            ...
            checkpoint_dict = torch.load(os.path.join(model_path, 'check_point_dict.pth'), map_location=self.args.device)
        ...
        for epoch in train_iterator:
            ...
            # if epoch % 3 == 0:
            #     logger.info('SHUFFLE PERMUTED DATA EVERY 3 epochs')
            #     self.train_dataset._reset()
            #     train_dataloader = self.get_train_dataloader()
    ...
    def _prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            with torch.no_grad():
                ...
                # step_eval_loss, logits = outputs[:2] # for only mlm loss
                # step_eval_loss, domain_logits, gate_logits = outputs[:3]  # added sop_scores for albert (modeling_albert.py line 697)
                # step_eval_loss = outputs['total_loss']

                    # slot_value_loss = outputs['slot_value_loss']

                for key, val in outputs.items():
                    # if 'list' in key:
                    #     for x in val:
                    #         # print(key, torch.isnan(x).any())
                    #         # if torch.isnan(x).any():
                    #             print(key, torch.where(torch.isnan(val)))
                    # elif 'logits' in key:
                    #     # print(key, torch.isnan(val).any())
            ...
            if not prediction_loss_only:
                ...
                # for i in range(4):
                #     # if (slot_preds_tmp[i] == slot_label_ids_tmp[i]).all() or cnt in [3, 4, 5, 6, 12, 7, 44, 100, 52]:
                #     # if cnt in [3, 4, 5, 6, 7, 9, 12,13, 14, 17, 19,21, 22, 27, 33, 44, 46, 63, 52, 53, 59, 60, 61, 63, 65, 69, 78,89, 90, 91, 96, 100, 123]:
                #
